...
int app_webrtc_trigger_offer(char *pPeerId)
{
    ...
    if (peerConnectionFound) {
        ...
        /* LOCAL PATCH (BitBang): create a data channel before the offer.
         *
         * Nothing in this SDK or its examples ever calls create_data_channel,
         * because it is built for media -- so a data-channel-only device
         * offers with no m=application section and the browser has nothing
         * to open. The offer is generated by set_callbacks below, so this is
         * the only window in which the channel can still reach the SDP.
         *
         * Only when no media is configured, to leave the camera examples
